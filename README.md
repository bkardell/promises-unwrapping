...
1. If `IsPromise(x)`, and `Get(x.[[GetInheritance]](), "constructor")` equals `C`, return `x`.
1. Otherwise,
...
1. Otherwise,
   1. Let `C` be `Get(this, "constructor")`.
...
1. If `IsConstructor(C)`,
   1. Let `resolver` be an ordinary function that:
      1. Lets `resolve` be the value it is passed as its first argument.
      1. Lets `reject` be the value it is passed as its second argument.
